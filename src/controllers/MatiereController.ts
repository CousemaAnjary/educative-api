...
module.exports = {
  ...
  async getMatieresByUserNiveau(req: Request, res: Response) {
    try {
      ...
      const matieres = await MatiereService.getMatieresByUserId(userId);
      if (!matieres || matieres.length === 0) return res.status(404).json({ success: false, message: "Aucune matière trouvée pour cet utilisateur" });

      return res.status(200).json({ success: true, matieres });
