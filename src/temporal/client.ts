...
async function main() {
  try {
    ...
    if (args.length < 2) {
      console.error("Usage: npm run client <workflow> <email> [password]");
      process.exit(1);
    }

    const [workflowName, email, password] = args;

    switch (workflowName) {
      case "synchronise": {
        if (!password) {
          console.error("Le mot de passe est requis pour la synchronisation.");
          process.exit(1);
        }
        const syncHandle = await startSynchronisationWorkflow(email, password);
        console.log(
